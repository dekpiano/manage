...
$(document).on("click", ".ShowEditRoomOnline", function() {
    ...
    $.post("../../teacher/ConTeacherTeaching/EditRoomOnline", { roomid: $(this).attr('roomid') }, function(data, status) {
...
$(document).on("submit", ".Update_RoomOnline", function(e) {
    e.preventDefault(e);
    $.ajax({
        url: '../../teacher/ConTeacherTeaching/UpdateRoomOnline',
        type: "post",
        data: new FormData(this), //this is formData
        processData: false,
        contentType: false,
...
$(document).on("submit", ".FormDeleteRoomOnline", function(e) {
    e.preventDefault(e);
    $.post("../../teacher/ConTeacherTeaching/DeleteRoomOnline", { roomid: $("#del_roomon_id").val() }, function(data, status) {
        if (data == 1) {
            Swal.fire({
                position: 'top-end',
                icon: 'success',
